  ...
  environment {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                cleanWs()
                git branch: 'develop', url: 'https://github.com/Shopping-App-Services/checkout-service.git'
            }
        }
            ...
            steps {
                script {
                  ...
                  withDockerRegistry(credentialsId: 'docker-token', toolName: 'docker') {
                        ...
                        sh "docker tag checkout-service nitesh2611/checkout-service:latest "
                        sh "docker push nitesh2611/checkout-service:latest "
                    }
                }
            }
        }
